...
# rows x cols
def matrix_dimensions(M: list[list[RealNumber]]):
    return (len(M), len(M[0]))

def matrix_product_entry(M: list[list[RealNumber]], N: list[list[RealNumber]], i: int, j: int):
    m, n = matrix_dimensions(M)
    n2, r = matrix_dimensions(N)
    if n != n2:
        raise Exception()
    result = 0
    for k in range(n):
...
def matrix_product(M: list[list[RealNumber]], N: list[list[RealNumber]]):
    m, n = matrix_dimensions(M)
    n2, r = matrix_dimensions(N)
    if n != n2:
        raise Exception()
    ...
    for i in range(m):
        for j in range(r):
            result[i][j] = matrix_product_entry(M, N, i, j)
    return result
try:
